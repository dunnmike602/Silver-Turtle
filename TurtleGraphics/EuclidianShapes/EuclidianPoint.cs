...
namespace TurtleGraphics.EuclidianShapes
{
    public class EuclidianPoint
    {
        ...
        public void ClampXToPlane(double planeStart, double planeEnd)
        {
            var clampedX = Math.Max(X, planeStart);
            clampedX = Math.Min(clampedX, planeEnd);
            X = clampedX;
        }

        public void ClampYToPlane(double planeStart, double planeEnd)
        {
            var clampedY = Math.Max(Y, planeStart);
            clampedY = Math.Min(clampedY, planeEnd);
            Y = clampedY;
        }
    }
}