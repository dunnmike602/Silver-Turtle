...
{
    public class TurtleColorViewModel : ViewModelDetailBase<TurtleColorViewModel, TurtleColor>, ITurtleColorViewModel
    {
        private readonly List<TurtleColor> _turtleColors = new List<TurtleColor>();

        private void PopulateColors()
        {
            const WellKnownColors wellKnownColors = WellKnownColors.Turquoise;

            var colors = ExtensionMethods.ConvertAll((uint[])wellKnownColors.GetValues(),
                                                     f => f.ToColor());

            foreach (var nextColor in colors)
            {
                _turtleColors.Add(new TurtleColor { Color = nextColor });
            }
        }

        public TurtleColorViewModel()
        {
            PopulateColors();
        }

        public List<TurtleColor> TurtleColors
        {
            get { return _turtleColors; }
        }
    }
}